...
file = Excel(CASE_FILE_PATH, "student_list")  # excel文件
...
class Testcase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        :return:
        """
        log.info("{:*^40s}".format("学生列表开始执行用例"))

    @classmethod
    def tearDownClass(cls):
        req.close()
        log.info("{:*^40s}".format("学生列表结束用例执行"))
    ...
    def test_case(self, data_dict):
        ...
        if headers:
            new_headers = json.loads(Context.student_list_parametrization(data_dict["headers"]))
        else:
            new_headers = {}
