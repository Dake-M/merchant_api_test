...
file = Excel(CASE_FILE_PATH, "report_center")  # excel文件
...
class Testcase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        :return:
        """
        log.info("{:*^40s}".format("报表中心开始执行用例"))

    @classmethod
    def tearDownClass(cls):
        req.close()
        log.info("{:*^40s}".format("报表中心结束用例执行"))
    ...
    def test_case(self, data_dict):
        ...
        if case_data:
            new_data = json.loads(Context.report_cente_parametrization(case_data))
            # log.info(new_data)
        else:
            new_data = {}
        ...
        if headers:
            new_headers = json.loads(Context.report_cente_parametrization(data_dict["headers"]))
